...
<head>
    ...
    <title>🚀 VenoM KiinG - UPTIME MONITOR</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 VenoM kiinG UPTIME MONITOR</h1>
            <p>Advanced URL Monitoring & Auto-Recovery System</p>
            <div style="margin-top: 15px;">
                <span class="alert alert-success">🟢 SYSTEM STATUS: {{ app_status }}</span>
            </div>
        </div>
        ...
        <div class="control-panel">
            <h2 style="margin-bottom: 20px; color: #4a00e0;">➕ Add New Monitor URL</h2>
            <form method="POST" action="/add_url">
                ...
                <button type="submit" class="btn">➕ Add to Monitoring</button>
            </form>
        </div>
        
        <div class="urls-list">
            <h2 style="margin-bottom: 20px; color: #4a00e0;">📊 Monitored URLs ({{ urls_count }}/100)</h2>
        ...
        <div style="margin-top: 30px; text-align: center; color: white;">
            <p>🔄 Auto Ping Every 30 Seconds | 🛠 Auto Recovery System | 📈 Max 100 URLs</p>
        </div>
    </div>
</body>
</html>
...
def add_url():
    ...
    if url not in MONITOR_URLS:
        ...
        print(f"✅ Added URL to monitoring: {name} ({url})")
    ...
    while True:
        try:
            for url in MONITOR_URLS[:]:  # Use slice copy for thread safety
                ...
                if status == 'UP':
                    print(f"✅ [{current_time}] {UPTIME_DATA[url]['name']} - UP ({response_time}ms)")
                else:
                    print(f"❌ [{current_time}] {UPTIME_DATA[url]['name']} - DOWN ({error})")
...
def auto_recovery(url):
    ...
    for attempt in range(max_retries):
        print(f"🔄 Auto-recovery attempt {attempt + 1} for {url}")
        time.sleep(retry_delay)
        
        status, response_time, error = ping_url(url)
        if status == 'UP':
            print(f"✅ Auto-recovery SUCCESS for {url}")
            return
    
    print(f"🚨 Auto-recovery FAILED for {url} after {max_retries} attempts")
...
def start_monitoring():
    ...
    print("🚀 Uptime Monitoring System Started!")
    print(f"📊 Monitoring {len(MONITOR_URLS)} URLs every 30 seconds")
...
def main():
    ...
    print("🔧 Original Server: http://localhost:4000")
    print("🔄 URL Monitoring: Active (30 second intervals)")
